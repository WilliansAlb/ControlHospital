 ...
 * @author yelbetto
 */
public class Inicio1 extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String sql = "SELECT COUNT(*) AS total FROM Usuarios WHERE id_usuario = ? AND password = ?";
        int pass = 0;
        if (cn.isPaso()) {
            try {
                cn.setPs(cn.getDb().prepareStatement(sql));
                cn.getPs().setString(1, user);
                cn.getPs().setString(2, pass2);
            ...
            if (pass != 0) {
                ...
                s.setAttribute("mensaje", "El nombre de usuario no existe");
                response.sendRedirect("/Hospital");
            }
        } else {
            s.setAttribute("mensaje", "Ha ocurrido un error con la base de datos, vuelve a intentar mas tarde");
            response.sendRedirect("/Hospital");
        }
    }
